...
import { calculateContrast, analyzeColorPair, suggestAccessibleColors } from './wcag-contrast.js';
// Create MCP server
    ...
    return {
        tools: [
            ...
            },
            {
                ...
                description: 'Analyze a color pair for WCAG compliance. Returns detailed analysis including whether it passes for normal text, large text, and UI components.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        foreground: {
    ...
    try {
        switch (name) {
            ...
            case 'analyze_color_pair': {
                const { foreground, background, contentType = 'normal-text', level = 'AA' } = args;
            ...
            case 'suggest_accessible_color': {
                const { foreground, background, targetRatio, preserve = 'both' } = args;
                const suggestions = suggestAccessibleColors(foreground, background, targetRatio, preserve);
                return {
                    content: [
                        {
                            type: 'text',
